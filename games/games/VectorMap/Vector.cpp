...
int16_t vectorArt::SHEILD[] = {
	TRIANGLE_FAN,
	255,255,0, 8, 0,0,3,2,4,6,2,5,0,6,-2,5,-4,6,-3,2
};

int16_t vectorArt::CASTLE_TURRET[] = {
	TRIFAN_3D,
...
void VectorMap::drawTile(PicoDisplay& pico_display, int32_t x, int32_t y, int32_t sz)
{
	drawTriangle(pico_display, Point(x, y), Point(x + sz * 4, y), Point(x + sz*2, y + sz));
	drawTriangle(pico_display, Point(x, y), Point(x + sz * 4, y), Point(x + sz*2, y - sz));
}
...
void VectorMap::drawMan(PicoDisplay& pico_display, int mandir, int32_t x, int32_t y, int32_t pw, int32_t walk, Pen clothes)
{
	if (mandir==0) {
		drawManX(pico_display, x, pw / 4, y, 3, 3, walk, clothes); 
	}
	else {
		drawManZ(pico_display, x, pw / 4, y, 3, 3, walk, clothes);
	}
}
void VectorMap::drawManZ(PicoDisplay& pico_display, int32_t x, int32_t z, int32_t y, int32_t w, int32_t h, int32_t d, Pen clothes)
{
	pico_display.set_pen(skin);
...
void VectorMap::drawManX(PicoDisplay& pico_display, int32_t x, int32_t z, int32_t y, int32_t w, int32_t h, int32_t d, Pen clothes)
{
	pico_display.set_pen(skin);
...
{
	skin = pico_display.create_pen(240, 120, 80);
	blue = pico_display.create_pen(0, 0, 120);
	grey = pico_display.create_pen(80, 80, 80);
	red = pico_display.create_pen(200, 0, 0);
	
	map.resize(mapsize*mapsize);
	
	for (int i=0; i<2000; i++) {
		map[fast_rand() % mapsize + (fast_rand() % mapsize) * mapsize] = 1;
	}
}
...
void VectorMap::update(PicoDisplay& pico_display)
{
	...
	int xt = pw/tilesize;
	int zt = ph/tilesize;
	for (int yy=(dy * ph); yy<(dy * ph + zt); yy++) 
	{
		for (int xx=dx * pw; xx<(dx * pw + xt); xx++)
		{
			if (map[xx+yy*mapsize]==1) {
				pico_display.set_pen(80, 80, 200);
				drawTile(pico_display, xx - xz.x, yy - xz.y, tilesize);
				//pico_display.clear();
			}
		}
	}
		...
		for (int xr = 0; xr < 3; xr++) {
			...
			for (int i = 0; i < c; i++) {
				...
				if (distrib == 0) {
					...
					drawTile(pico_display, tx, ty, sz);
	...
	if (abs(adx)>1 || abs(ady)>1) {
		if ((x+adx)>-maplimit && (x+adx)<maplimit) x += adx;
		if ((z+ady)>-maplimit && (z+ady)<maplimit) z -= ady;
		mandir = (adx > 0 && ady > 0) ? 0 : 1;
		walk += wd;
	...
	*/
    ...
    drawMan(pico_display, mandir, -7, -3, pw, walk, grey);
	drawMan(pico_display, mandir, 0, 0, pw, walk, blue);
    drawMan(pico_display, mandir, 7, 5, pw, walk, red);
    drawMan(pico_display, mandir, -2, 9, pw, walk, grey);
    
	//x = x + ((buttonsPressed == 1 && x>-maplimit) ? -1 : (buttonsPressed == 4 && x<maplimit) ? 1 : 0);
	//z = z + ((buttonsPressed == 8 && z>-maplimit) ? -1 : (buttonsPressed == 2 && z<maplimit) ? 1 : 0);
