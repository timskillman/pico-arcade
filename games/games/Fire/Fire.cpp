...
void Fire::update(PicoDisplay& pico_display) 
{
    ...
    if (!y_pressed && pico_display.is_pressed(PicoDisplay::Y)) {
        wind++;
    }
    y_pressed = pico_display.is_pressed(PicoDisplay::Y);

    if (!b_pressed && pico_display.is_pressed(PicoDisplay::B)) {
        wind--;
    }
    b_pressed = pico_display.is_pressed(PicoDisplay::B);
    ...
    a_pressed = pico_display.is_pressed(PicoDisplay::A);

    //if (!x_pressed && pico_display.is_pressed(PicoDisplay::X)) {
    //    show_fps = !show_fps;
    //}
    //x_pressed = pico_display.is_pressed(PicoDisplay::X);
}
